...
- **♡ 記録キャッシュ表示ページ** (`/records-cache`)：ページ入口、收藏記録、localStorage 履歴、ビルド時キャッシュをクラウドタグと 2 列カードで表示し、中継アクセスと收藏/取消に対応します。
...
- ♡ **記録キャッシュ表示ページを追加**：`/records-cache` でページ入口、收藏、履歴、キャッシュ索引を集約し、各独立ページの上部に收藏/取消ボタンを追加しました。
