...
- 🧭 **404 推奨入口のランダム更新**：`/404` の「ここへ行きたいかも」一覧はグローバルナビゲーションと Web Components ライブラリ入口から抽出し、4 秒ごとにランダム更新し、手動更新ボタンも追加しました。
- 📬 **連絡センターを追加** (`/contact`)：QQ 連絡、メール連絡、QQ MailMe の 3 つの快捷入口を専用ページに集約し、選択した連絡方法をハイライト表示します。
- 🐾 **看板娘のフォールバック表示を修正**：崩れて見える猫顔文字を安定した肉球アイコンに変更し、読み込み失敗時の表示を分かりやすくしました。
