...
interface UseSearchReturn {
  ...
  activeSearchCat: Ref<string>
  activeEngineId: Ref<string>
  selectSearchCategory: (catId: string) => void
  activeCategory: ComputedRef<SearchCategory>
  activeEngine: ComputedRef<SearchEngine>
  clearSearch: () => void
  saveSearchQuery: (query: string) => void
  triggerSearch: () => void
  useHistoryTag: (tag: string) => void
  clearHistory: () => void
  filteredTools: ComputedRef<SearchToolItem[]>
}

export function useSearch(menuItems: Ref<NavigationCategory[]>, getCurrentTools: () => ToolItem[]): UseSearchReturn {
  const searchQuery = ref<string>('')
  const searchHistory = ref<string[]>(JSON.parse(localStorage.getItem('searchHistory') || '[]') as string[])
  ...
  const activeSearchCat = ref<string>('common')
  const activeEngineId = ref<string>('site')

  const selectSearchCategory = (catId: string): void => {
    activeSearchCat.value = catId
    const cat: SearchCategory | undefined = searchCategories.find((c: SearchCategory): boolean => c.id === catId)
    if (cat && cat.engines.length > 0) {
      activeEngineId.value = cat.engines[0].id
    }
  }

  const activeCategory = computed<SearchCategory>(() => {
    return searchCategories.find((c: SearchCategory): boolean => c.id === activeSearchCat.value) || searchCategories[0]
  })

  const activeEngine = computed<SearchEngine>(() => {
    const engines: SearchEngine[] = activeCategory.value.engines
    return engines.find((e: SearchEngine): boolean => e.id === activeEngineId.value) || engines[0]
  })
  ...
  const saveSearchQuery = (query: string): void => {
    ...
    if (!trimmed) return
    let history: string[] = searchHistory.value.filter((item: string): boolean => item !== trimmed)
  ...
  const triggerSearch = (): void => {
    ...
    if (activeEngine.value.id === 'site') {
      ...
      const url: string = activeEngine.value.url + encodeURIComponent(query)
      window.open(url, '_blank')
    }
  }

  const useHistoryTag = (tag: string): void => {
  ...
  const filteredTools = computed<SearchToolItem[]>(() => {
    if (!searchQuery.value) return getCurrentTools()
    const query: string = searchQuery.value.toLowerCase()
    
    const results: SearchToolItem[] = []
    menuItems.value.forEach((category: NavigationCategory): void => {
      if (category.tools) {
        category.tools.forEach((tool: ToolItem): void => {
      ...
      if (category.subcategories) {
        category.subcategories.forEach((sub: ToolSubcategory): void => {
          sub.tools.forEach((tool: ToolItem): void => {
            if (tool.name.toLowerCase().includes(query) || tool.desc.toLowerCase().includes(query)) {
              results.push({
                ...tool,
  ...
  const startVoiceSearch = (): void => {
    const SpeechRecognitionCtor: SpeechRecognitionConstructor | undefined = window.SpeechRecognition || window.webkitSpeechRecognition || window.mozSpeechRecognition || window.msSpeechRecognition
    ...
    try {
      ...
      recognition.onerror = (event: SpeechRecognitionErrorEvent): void => {
        ...
        let errorMsg: string = '语音识别失败，请检查麦克风权限！'
