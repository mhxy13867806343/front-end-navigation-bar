...
const createThemeItem = (i: number): AuthThemeItem => {
  ...
  return {
    ...
    icon: icons[catIdx],
    description: `100,000 范例库中专为 ${catName} 打造的 ${modeName} 交互面板范例卡片 #${i}，支持表单即时验证。`,
    features: ['高并发虚拟渲染', '100k 级数据索引', '防伪校验与安全拦截', '社交账号一键极速登录']
  }
}

// 动态生成 100,000 项范例数据集
const TOTAL_COUNT = 100000
const generate100kThemes = (): AuthThemeItem[] => {
...
const allThemes = ref<AuthThemeItem[]>(generate100kThemes())

// Filter State
const searchQuery = ref<string>('')
const selectedCategory = ref<string>('全部 100,000 款')
const selectedModeFilter = ref<'all' | 'login' | 'register'>('all')

// 分页与跳转 State (按每页 50 条分页，共 2,000 页)
...
const filteredThemes = computed(() => {
  ...
  return allThemes.value.filter((item) => {
    const matchCat = selectedCategory.value === '全部 100,000 款' || item.category === selectedCategory.value
  ...
  <div class="auth-showcase-page">
    ...
    <header class="showcase-header">
      <div class="header-inner">
        <div class="brand-title">
          <span class="badge-tag">🔐 Auth 100,000 Showcase</span>
          <h1>100,000 款精美登录与注册 UI 展厅</h1>
        </div>
      ...
      <div class="header-stats">
        <div class="stat-badge">
          <strong>100,000 款</strong> 海量 10 万 Auth 范例库
        </div>
        <div class="stat-badge">
          <strong>50,000 登录 + 50,000 注册</strong> 专精模式匹配
        </div>
        <div class="stat-badge">
    ...
    <div class="filter-control-bar">
      <div class="search-box">
        ...
        <input v-model="searchQuery" type="text" placeholder="搜索 100,000 款登录 / 注册 UI 范例..." @input="currentPage = 1" />
      </div>
      ...
      <div style="display: flex; gap: 8px; align-items: center;">
        <div style="display: flex; gap: 4px; align-items: center; background: rgba(30,41,59,0.8); padding: 4px 8px; border-radius: 10px; border: 1px solid rgba(255,255,255,0.1);">
          ...
          <input v-model.number="jumpIdInput" type="number" placeholder="1-100000" style="width: 80px; padding: 4px 8px; border-radius: 6px; border: 1px solid #334155; background: #0f172a; color: #fff; font-size: 0.8rem;" @keyup.enter="jumpToItemId" />
    ...
    <main class="showcase-stage">
      <div class="stage-card-wrapper" :class="activeTheme.styleCssClass">
        ...
        <!-- 2. 📝 注册 Form 展示 (满足用户 50,000/100,000 款专精注册界面) -->
