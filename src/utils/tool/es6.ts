...
export function deepClone<T>(source: T): T {
  ...
  if (Array.isArray(source)) {
    ...
    source.forEach((item: unknown): void => { cloneArr.push(deepClone(item)) })
    return cloneArr as T
  }

  // Handle Object
  const sourceObj: Record<string, unknown> = source as Record<string, unknown>
  const cloneObj: Record<string, unknown> = {}
  Object.keys(sourceObj).forEach((key: string): void => {
 ...
 */
export function debounceDemo(taskName: string, delay: number = 500): string {
 ...
 */
export function throttleDemo(taskName: string, limit: number = 500): string {
