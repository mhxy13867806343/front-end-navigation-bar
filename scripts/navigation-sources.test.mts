...
test('GitHub Pages production routing uses hash history and restores direct links to hash routes', () => {
  ...
  assert.match(notFoundSource, /__gh_pages_ts/)
  assert.match(notFoundSource, /Date\.now\(\)/)
...
test('Unknown SPA routes redirect to the redesigned 404 page with the original path preserved', () => {
  ...
  assert.match(notFoundPageSource, /import \{ menuItemsList \} from '@\/utlis\/menuItems'/)
  assert.match(notFoundPageSource, /import \{ webLibraryGroups/)
  assert.match(notFoundPageSource, /const RECOMMENDATION_REFRESH_MS = 4000/)
  assert.match(notFoundPageSource, /visibleRecommendedRoutes/)
  assert.match(notFoundPageSource, /function buildGlobalRecommendedRoutes/)
  assert.match(notFoundPageSource, /window\.setInterval\(refreshRecommendations, RECOMMENDATION_REFRESH_MS\)/)
  assert.match(notFoundPageSource, /class=["']route-refresh-button["']/)
...
test('Home farewell dialog opens only on the production homepage with live jokes and mail contact', () => {
  ...
  assert.match(appSource, /import HomeFarewellDialog from '\.\/components\/HomeFarewellDialog\.vue'/)
  assert.match(appSource, /<HomeFarewellDialog\s+v-if=["']isDyFormRoute && isProd["']\s+:route-path=["']route\.path["']/)
  assert.match(dialogSource, /preset=["']dialog["']/)
  assert.match(dialogSource, /import \$ from ['"]jquery['"]/)
  assert.match(dialogSource, /import\.meta\.env\.PROD/)
  ...
  assert.match(dialogSource, /class=["']joke-toolbar["']/)
  assert.match(dialogSource, /class=["']joke-count-label["']/)
...
test('Global anime mascot loads local Live2D widget assets on every route', () => {
  ...
  assert.match(mascotSource, /<span class=["']mascot-face["']>🐾<\/span>/)
  assert.doesNotMatch(mascotSource, /ฅ\^•ﻌ•\^ฅ/)
...
test('Home page keeps quick-entry clock without floating QQ and email contact buttons', () => {
  ...
  const routerSource = readFileSync(new URL('../src/router/index.ts', import.meta.url), 'utf8')
  const contactSource = readFileSync(new URL('../src/views/contact/index.vue', import.meta.url), 'utf8')
  const contactStyleSource = readFileSync(new URL('../src/views/contact/css/index.scss', import.meta.url), 'utf8')
  const webLibrarySource = readFileSync(new URL('../src/constants/webLibrary/pagesAndGames.ts', import.meta.url), 'utf8')

  assert.match(appSource, /mqqwpa:\/\/im\/chat\?chat_type=wpa&uin=869710179&version=1&src_type=web/)
  assert.match(routerSource, /path:\s*'\/contact'/)
  assert.match(routerSource, /views\/contact\/index\.vue/)
  assert.match(appSource, /routeViewPaths:[\s\S]*?\/contact/)
  assert.match(appSource, /'\/contact': '联系作者中心'/)
  assert.match(appSource, /const goContact = \(channel\?: string\): Promise<void \| Error> =>/)
  assert.match(appSource, /case 'qq':[\s\S]*?await goContact\('qq'\)/)
  assert.match(appSource, /case 'email':[\s\S]*?await goContact\('email'\)/)
  assert.match(appSource, /case 'qq-mailme':[\s\S]*?await goContact\('qq-mailme'\)/)
  assert.match(appSource, /<el-dropdown-item command="contact">联系作者中心<\/el-dropdown-item>/)
  assert.match(contactSource, /const contactActions: ContactAction\[\] = \[/)
  assert.match(contactSource, /QQ 联系/)
  assert.match(contactSource, /邮箱联系/)
  assert.match(contactSource, /QQ 邮我/)
  assert.match(contactSource, /activeChannel/)
  assert.match(contactSource, /mqqwpa:\/\/im\/chat/)
  assert.match(contactSource, /mailto:\$\{CONTACT_EMAIL\}/)
  assert.match(contactSource, /navigator\.clipboard\?\.writeText/)
  assert.match(contactStyleSource, /\.contact-grid/)
  assert.match(contactStyleSource, /prefers-reduced-motion:\s*reduce/)
  assert.match(webLibrarySource, /📬 联系作者中心/)
...
test('WebLibrary popover provides global search with search history storage', () => {
  ...
  assert.match(noticeSource, /confirmClearSearchHistory/)
  assert.match(noticeSource, /onTransferSelectChange/)
  assert.match(noticeSource, /ElMessageBox\.confirm/)
  assert.match(noticeSource, /el-select/)
  assert.match(noticeStyleSource, /\.global-search-select/)
  assert.match(noticeStyleSource, /\.transfer-search-select/)
  assert.match(noticeStyleSource, /\.select-dropdown-header/)
...
test('MyGithubProjects page provides 21 open source projects showcase with route and menu entry', () => {
  const routerSource = readFileSync(new URL('../src/router/index.ts', import.meta.url), 'utf8')
  const appSource = readFileSync(new URL('../src/App.vue', import.meta.url), 'utf8')
  const pageSource = readFileSync(new URL('../src/views/myGithubProjects/index.vue', import.meta.url), 'utf8')
  const pageStyleSource = readFileSync(new URL('../src/views/myGithubProjects/css/index.scss', import.meta.url), 'utf8')
  const dataStoreSource = readFileSync(new URL('../src/constants/githubProjectsData.ts', import.meta.url), 'utf8')
  const webLibrarySource = readWebLibrarySource()

  assert.match(routerSource, /path:\s*'\/my-github-projects'[\s\S]*?name:\s*'MyGithubProjects'/)
  assert.match(appSource, /routeViewPaths:[\s\S]*?'\/my-github-projects'/)
  assert.match(webLibrarySource, /command:\s*'\/my-github-projects'/)
  assert.match(webLibrarySource, /🐙 个人开源项目库/)
  assert.match(dataStoreSource, /ai-credit-limit/)
  assert.match(dataStoreSource, /layui-vue3-admin-ts/)
...
test('OSChina reading list page provides weekly and monthly hot articles from oschina.net', () => {
  ...
  assert.match(pageSource, /api-apiv1-oschina/)
  assert.match(pageSource, /activePeriod/)
  assert.match(pageSource, /activeType/)
  assert.match(pageSource, /周榜/)
  assert.match(pageSource, /月榜/)
  assert.match(pageSource, /key:\s*'all'/)
  assert.match(pageSource, /key:\s*'news'/)
  assert.match(pageSource, /key:\s*'blog'/)
  assert.match(pageSource, /:active="isContentItemFavorite\(item\.link\)"/)
  assert.match(pageSource, /toggleContentItemFavorite\(\{ id: item\.link, title: item\.title, url: item\.link, source: '开源中国' \}\)/)
