...
test('Juejin course page loads booklet list and keeps course-list links', () => {
  ...
  assert.match(pageSource, /COURSE_API = '\/api-juejin\/booklet_api\/v1\/booklet\/listbycategory'/)
  assert.match(pageSource, /CATEGORY_API = '\/api-juejin\/tag_api\/v1\/query_category_briefs\?show_type=1'/)
  ...
  assert.match(pageSource, /fallbackCourseCategories[\s\S]*?category_name:\s*'后端'[\s\S]*?category_name:\s*'前端'[\s\S]*?category_name:\s*'Android'[\s\S]*?category_name:\s*'iOS'[\s\S]*?category_name:\s*'人工智能'[\s\S]*?category_name:\s*'开发工具'[\s\S]*?category_name:\s*'代码人生'[\s\S]*?category_name:\s*'阅读'/)
  assert.match(pageSource, /const SORT_PRICE_ASC = 8[\s\S]*?const SORT_PRICE_DESC = 9/)
  assert.match(pageSource, /function togglePriceSort\(\): void \{[\s\S]*?SORT_PRICE_DESC \? SORT_PRICE_ASC : SORT_PRICE_DESC/)
  ...
  assert.match(pageSource, /const courseCacheUrl = `\$\{import\.meta\.env\.BASE_URL\}live-data\/juejin-course-cache\.json`/)
  ...
  assert.match(packageSource, /generate-juejin-rank-cache\.mts && node scripts\/generate-juejin-course-cache\.mts && vite build/)
  assert.match(cacheScriptSource, /publicCachePath[\s\S]*?public[\s\S]*?live-data[\s\S]*?juejin-course-cache\.json/)
  ...
  assert.match(pageSource, /https:\/\/juejin\.cn\/user\/\$\{userId\}/)
  assert.match(pageSource, /https:\/\/juejin\.cn\/book\/\$\{bookletId\}\?utm_source=course_list/)
})
