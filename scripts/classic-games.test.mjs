...
const [appSource, menuSource] = await Promise.all([
  readFile(new URL('../src/App.vue', import.meta.url), 'utf8'),
  readFile(new URL('../src/utlis/menuItems.js', import.meta.url), 'utf8'),
])

const onlineWorksSource = menuSource.slice(
  menuSource.indexOf('export const onlineWorksList'),
  menuSource.indexOf('export const authorWorksList'),
)
...
test('App imports and assigns every classic canvas game', () => {
  for (const { type, component } of classicGames) {
    ...
    assert.match(
      appSource,
      new RegExp(`case\\s+["']${type}["']\\s*:\\s*currentGame\\.value\\s*=\\s*${component}`),
      `${type} should assign ${component}`,
    )
  }
})
