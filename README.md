...
- **♡ 记录缓存展示页** (`/records-cache`)：统一展示页面入口、收藏记录、localStorage 历史与构建缓存，支持云标签、双列卡片、中转访问和收藏/取消收藏。
...
- ♡ **新增记录缓存展示页**：`/records-cache` 以云标签和双列卡片汇总页面入口、收藏、历史记录与缓存索引，所有独立页面顶部新增收藏/取消收藏按钮。
