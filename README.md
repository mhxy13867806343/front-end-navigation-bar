...
- 🧭 **404 推荐入口随机刷新**：`/404` 的“也许你想去这里”从全局导航与 Web 组件库入口抽取，每 4s 自动随机换一批，并提供手动“换一批”按钮。
- 📬 **新增联系作者中心** (`/contact`)：将“QQ 联系 / 邮箱联系 / QQ 邮我”三个快捷入口收进独立页面，快捷入口点击后会进入页面并高亮对应联系方式。
- 🐾 **修复看板娘离线状态**：去掉异常猫脸字符，离线/加载失败时改为稳定的爪印图标与清晰提示。
...
- 🌊 **404 水波浪上浮页面**：访问不存在页面会进入 `/404`，主视觉加入从底部向上浮现的水波浪、气泡和 404 数字水光裁剪动效，并保留返回首页、返回上一页和入口搜索。
